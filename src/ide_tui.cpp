...
void EmulatorIDETUI::drawEditor(int h, int w) {
    ...
    if (screenCursorY >= 1 && screenCursorY < editorHeight && screenCursorX >= 5 &&
        screenCursorX < w) {
...
void EmulatorIDETUI::drawCode(int h, int w) {
    ...
    for (int i = 0; i < linesAvail && first + i < (int)prog.size(); ++i) {
        ...
        std::string line =
            std::string(1, isBP ? '*' : ' ') + std::to_string(idx) + ": " + labelPrefix + prog[idx];
...
void EmulatorIDETUI::drawRegisters(int y, int x, int w) {
    ...
    mvprintw(
        y++, x, "AX=%04X (AH=%02X AL=%02X)  BX=%04X", r.AX.x, r.AX.bytes.h, r.AX.bytes.l, r.BX.x);
    mvprintw(
        y++, x, "CX=%04X (CH=%02X CL=%02X)  DX=%04X", r.CX.x, r.CX.bytes.h, r.CX.bytes.l, r.DX.x);
    ...
    mvprintw(y++,
             x,
             "FLAGS=%04X [O=%d D=%d I=%d T=%d S=%d Z=%d A=%d P=%d C=%d]",
             r.FLAGS,
             !!(r.FLAGS & Registers::OF),
             !!(r.FLAGS & Registers::DF),
             !!(r.FLAGS & Registers::IF),
             !!(r.FLAGS & Registers::TF),
             !!(r.FLAGS & Registers::SF),
             !!(r.FLAGS & Registers::ZF),
             !!(r.FLAGS & Registers::AF),
             !!(r.FLAGS & Registers::PF),
             !!(r.FLAGS & Registers::CF));
}
...
void EmulatorIDETUI::drawStatus(int h, int w) {
    ...
    if (currentMode == 0) {
        status +=
            " | Line: " + std::to_string(cursorRow + 1) + ", Col: " + std::to_string(cursorCol + 1);
    } else {
...
void EmulatorIDETUI::run() {
    setStatus(
        "F1=Help F2=Compile(WIP) F3=Debug F5=Run F10=Step Ctrl+S=Save Ctrl+O=Open Ctrl+N=New");
...
void EmulatorIDETUI::handleEditorInput(int ch) {
    switch (ch) {
        case KEY_F(1):
            ...
            setStatus(
                "F1=Help F2=Compile(WIP) F3=Debug F5=Run F10=Step Ctrl+S=Save Ctrl+O=Open "
                "Ctrl+N=New");
            break;
        case KEY_F(2):
            compileAndLoad();
...
void EmulatorIDETUI::handleDebuggerInput(int ch) {
    switch (ch) {
        ...
        case KEY_DOWN:
            memWindowStart =
                std::min((int)emulator->getMemory().size() - memWindowSize, memWindowStart + 16);
            break;
        case KEY_PPAGE:
